...
class Tree
{
    ...
    public function get_parent($myid)
    {
        ...
        $pid = $this->arr[$myid][$this->config['pid']];//父级id
        $pid = $this->arr[$pid][$this->config['pid']];//祖级id
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if ($a[$this->config['pid']] == $pid)
                    $newarr[$id] = $a;
            }
        }
        return $newarr;
    ...
    public function get_pos($index, &$newarr)
    {
        ...
        $pid = $this->arr[$index][$this->config['pid']];
    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '')
    {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                @extract($value);
                $pid == 0 && $str_group ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");//顶级
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
