...
/* 公司是否发布 */
$(function () {
    $('body').on('click','.display-btn1',function () {
        var $url = this.href,
            val = $(this).data('id'),
            $btn=$(this);
        ...
        $.post($url, {x: val}, function (data)
        {
            if (data.code==1)
            {
                if (data.msg == 0)
                {
                    var a = '<button class="btn btn-minier btn-yellow">显示</button>';
                    $btn.children('div').html(a).attr('title','已显示');
                    return false;
                }
                else
                {
                    var b = '<button class="btn btn-minier btn-danger">隐藏</button>';
...
$(function () {
    $('.adminform').ajaxForm({
        ...
        beforeSubmit: '', // 此方法主要是提交前执行的方法，根据需要设置
        success: complete, // 这是提交后的方法
        dataType: 'json'
    });
});
...
function ajax_page(page) {
    ...
    //request_module=&request_controller=&request_action=&request_method=&page=3
    //reservation=&key=&page=2
    //alert($('#list-filter').serialize()+'&page='+page);
	$.ajax({
		type:"POST",
