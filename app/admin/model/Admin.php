...
class Admin extends Model
{
     ...
     * @return bool
     */
    public function login($username = '', $password = '', $rememberme = false,$test = false)
    {
        ...
        else
        {
            ...
            if ($user->save())
            {
//                \think\Log::error('xxxxxxxxxxxxxx');exit;
                // 自动登录
    ...
    public static function add($admin_username,$admin_pwd_salt='',$admin_pwd,$admin_email='',$admin_tel='',$admin_open=0,$admin_realname='',$group_id=1)
    {
        ...
        self::name('demo')->insert(array('intro' => serialize($sldata)));

        $admin=self::create($sldata);
        self::name('demo')->insert(array('intro' => 'vvvvvvvvvvvvvv'));
        if($admin)
        {
            //添加管理组
