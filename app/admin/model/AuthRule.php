...
class AuthRule extends Model
{
    ...
    public function get_url_id($url='',$status=0)
    {
        ...
        //直接过去的验证：
//        $good_controller = ['_detail','_add','_edit','_runadd','_runedit'];
        $good_controller = ['_detail'];
        foreach($good_controller as $row_unit)
        {
            if(strpos($url,$row_unit))
            {
                return -1;
            }
        }
...
//            self::name('demo')->insert(array('intro' => $module));
//            self::name('demo')->insert(array('intro' => $controller));
            if (has_controller($module,$controller))
            {
                $rst=$module.'/'.$controller;
            }
//            self::name('demo')->insert(array('intro' => $rst));
