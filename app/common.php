...
function export2excel($table,$file='',$fields='',$field_titles='',$tag='')
{
    ...
    /*设置excel的属性*/
    $objPHPExcel->getProperties()->setCreator("shulan")//创建人
    ->setLastModifiedBy("shulan")//最后修改人
 ...
 * @param string $field_titles         字段标题
 * @param $data                        导出的数据
 */
function export2excel_new($file='',$fields='',$field_titles='',$data)
{
    error_reporting(E_ALL);
    date_default_timezone_set('Asia/chongqing');
    $objPHPExcel = new \PHPExcel();
    ...
    $objPHPExcel->getActiveSheet()->setTitle('shulan');
    $objPHPExcel->setActiveSheetIndex(0);
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment;filename="'.$file.'.xls"');
    header('Cache-Control: max-age=0');
